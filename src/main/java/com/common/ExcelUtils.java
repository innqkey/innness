...
* 类说明：
* @author 
* @version 创建时间：2018年3月23日 下午7:47:53
* 
*/
    ...
    public static Map<String, Object> fillExcelData(  
        ...
        for (Object object : objects) {  
            ...
            for (String key : keySet) {  
                ...
                String userData;
				if ("申请时间".equals(key)) { 
					Date object2 = (Date) userField.get(object);
				    userData = DateUtils.format(object2, "yyyy-MM-dd HH:mm:ss");          	
				 }else {
					userData = String.valueOf(userField.get(object));  
					 // 一些特殊的判断  
	                if ("银行卡类型".equals(key)) {  
	                	userData = DictConConstant.getDicName("BankType", userData);
	                }  
	                if ("提现状态".equals(key)) {  
	                	userData = DictConConstant.getDicName("AuditingStatus", userData);
	                }  
				 }
                data.add(userData);  
            }  
